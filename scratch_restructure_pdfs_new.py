...
# 1. Update the individual topic pages
for file_path, injection_html in mapping.items():
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as f:
            content = f.read()
        
        # Only inject if not already there
        if "เอกสารดาวน์โหลด" not in content:
            # Inject right before </main>
            new_content = content.replace("</main>", injection_html + "\n    </main>")
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(new_content)
            print(f"Updated {file_path}")
        else:
            print(f"Skipped {file_path} (already has เอกสารดาวน์โหลด)")
    else:
        print(f"Warning: {file_path} not found")

# 2. Update ศูนย์รวมการดาวน์โหลด.html to only keep the reports
dl_file = "ศูนย์รวมการดาวน์โหลด.html"
if os.path.exists(dl_file):
    with open(dl_file, "r", encoding="utf-8") as f:
        dl_content = f.read()
    
    # We will replace the entire <div class="download-list">...</div> with a hardcoded version containing only the reports
    new_dl_html = """<div class="download-list">
    <div>
    ...
    # Use regex to replace the <div class="download-list"> block
    # It starts at <div class="download-list"> and ends before </div>\n    </main>
    pattern = re.compile(r'<div class="download-list">.*?(?=</div>\s*</main>)', re.DOTALL)
    new_content = pattern.sub(new_dl_html, dl_content)
    
    with open(dl_file, "w", encoding="utf-8") as f:
        f.write(new_content)
    print(f"Updated {dl_file}")

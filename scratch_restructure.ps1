...
foreach ($key in $mapping.Keys) {
    if (Test-Path $key) {
        $content = Get-Content $key -Raw -Encoding UTF8
        if ($content -notmatch "เอกสารดาวน์โหลด") {
            $injection = $mapping[$key]
            $newContent = $content -replace "</main>", "$injection`n    </main>"
            Set-Content -Path $key -Value $newContent -Encoding UTF8
            Write-Host "Updated $key"
        } else {
            Write-Host "Skipped $key (already has เอกสารดาวน์โหลด)"
        }
    } else {
        Write-Host "Warning: $key not found"
    }
}

$dlFile = "ศูนย์รวมการดาวน์โหลด.html"
if (Test-Path $dlFile) {
...
"@
    ...
    $pattern = '<div class="download-list">.*?(?=</div>\s*</main>)'
    $newContent = [regex]::Replace($dlContent, $pattern, $newDlHtml, [System.Text.RegularExpressions.RegexOptions]::Singleline)
    
    Set-Content -Path $dlFile -Value $newContent -Encoding UTF8
    Write-Host "Updated $dlFile"
}

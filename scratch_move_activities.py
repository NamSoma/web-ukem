...
with open(news_th, 'r', encoding='utf-8') as f:
    c = f.read()
    # Find the end of news-grid. There is <!-- Activity 3 --> ... </div> then a closing </div> for news-grid.
    if '<!-- Activity 6 -->' not in c:
        c = re.sub(r'(<!-- Activity 3 -->.*?</div>\s*</div>)', r'\1\n' + cards_th, c, flags=re.DOTALL)
...
# 3. Clean up TH safety page
safety_th = os.path.join(base_dir, "สังคม", "อาชีวอนามัยและความปลอดภัย.html")
with open(safety_th, 'r', encoding='utf-8') as f:
    c = f.read()
# Replace from the stray </div> down to just before <alert-card>
pattern_th = r'\s*</div>\s*<div class="project-card">.*?</div>\s*</div>\s*</div>\s*(?=<alert-card>)'
c = re.sub(pattern_th, '\n\n            ', c, flags=re.DOTALL)
with open(safety_th, 'w', encoding='utf-8') as f:
    f.write(c)

# 4. Clean up EN safety page
safety_en = os.path.join(base_dir, "en", "สังคม", "อาชีวอนามัยและความปลอดภัย.html")
with open(safety_en, 'r', encoding='utf-8') as f:
    c = f.read()
# Replace similarly
pattern_en = r'\s*</div>\s*<div class="project-card">.*?</div>\s*</div>\s*</div>\s*(?=<alert-card>)'
c = re.sub(pattern_en, '\n\n            ', c, flags=re.DOTALL)
with open(safety_en, 'w', encoding='utf-8') as f:
    f.write(c)

print("Done moving activities")
